...
function stem(token) {
	...
	if (l > 5 && token.substring(l - 5) === 'aliti' && hasVowel(token, 5)) {
		token = token.substring(0, l - 3);
		
		l = l - 3;
	}

	if (l > 5 && token.substring(l - 5) === 'iviti' && hasVowel(token, 5)) {
		token = token.substring(0, l - 3) + 'e';

		l = l - 2;
	}

	if (l > 5 && token.substring(l - 3) === 'ate' && hasVowel(token, 3)) {
		token = token.substring(0, l - 3);

		l = l - 3;
	}

	if (l > 5 && token.substring(l - 3) === 'iti' && hasVowel(token, 3)) {
		token = token.substring(0, l - 3);

		l = l - 3;
	}

	if (l > 5 && token.substring(l - 3) === 'ize' && hasVowel(token, 3)) {
		token = token.substring(0, l - 3);
